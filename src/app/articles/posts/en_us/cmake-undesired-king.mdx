...
This is where CMake \(or Cross Platform Makefile Generator\) steps in. Instead of writing and maintaining separate build files for each platform, you define your project once using CMake, with the file `CMakeLists.txt`. From that single configuration, CMake can generate the proper build files for a variety of systems, such as:
...
![CMake's Design](https://media.githubusercontent.com/media/bmarques1995/portfolio_album/refs/heads/main/posts/cmake/Single_Source_Build.png)

So, is CMake basically a middleware for other build systems? Exactly — you nailed it. Remember: one key concept in learning is to recognize similarities, which makes it easier to understand a concept.
